...
export const ProductSearch: React.FC<ProductSearchProps> = ({
  ...
  return (
    <div className="bg-slate-900 border border-slate-800 rounded-2xl p-5 shadow-xl space-y-4">
              ...
              {filteredProducts.length === 0 ? (
                ...
                filteredProducts.map((p) => (
                  <div
                    ...
                    className="p-2.5 hover:bg-slate-800 cursor-pointer flex items-center justify-between transition"
                  >
                    ...
                    <button
                      ...
                      className="px-2 py-1 rounded text-[11px] font-bold bg-emerald-600 hover:bg-emerald-500 text-white"
                    >
                      + Tambah
                    </button>
                  </div>
      ...
      {selectedProduct && (
        <div className="p-3 bg-slate-950 rounded-xl border border-emerald-500/30 flex flex-wrap items-center justify-between gap-2 text-xs">
          ...
          <div className="text-[11px] text-slate-400 flex items-center gap-3">
            <span>
              Asal: <strong className="text-emerald-300 font-mono">{selectedProduct.defaultStorageFrom || 'STORAGE-A1'}</strong>
            </span>
            <ArrowRight className="w-3 h-3 text-slate-500" />
            <span>
              Tujuan: <strong className="text-emerald-300 font-mono">{selectedProduct.defaultStorageTo || 'DISPLAY-01'}</strong>
            </span>
          </div>
        </div>
      )}
